...
Object.assign(app, {
    ...
    state: typeof app.state !== 'undefined' ? app.state : { // State bütünlüğü koruması
        ...
        takasDayActive: false, // Mahalle Borsası / Takas Günü modu aktif mi?
        currentOfferType: 'cash',
        selectedSwapProductId: null,
        shareViewMode: 'map', // 'list' or 'map'
    ...
    init: function () {
        ...
        if (typeof db !== 'undefined' && db.enablePersistence) {
            db.enablePersistence({ synchronizeTabs: true }).catch(err => {
                ...
                } else if (err.code == 'failed-persistence') {
                    console.warn("Persistence already enabled or failed.");
                } else {
        ...
        // --- Mahalle Pazarı Buton Event Listener ---
        const pazarAddBtn = document.getElementById('pazar-add-btn');
        if (pazarAddBtn) {
            pazarAddBtn.addEventListener('click', () => {
                console.log('[Pazar Modal] Buton tıklandı (addEventListener)');
                this.openPazarAddModal();
            });
            console.log('[Pazar Modal] Event listener başarıyla eklendi');
        } else {
            console.warn('[Pazar Modal] pazar-add-btn elementi bulunamadı (init sırasında)');
        }
    },
    ...
    showScreen: function (screenId) {
        ...
        if (screenId === 'ar') this.initARMode();
    ...
    renderGallery: function () {
        ...
        wrapper.innerHTML = items.map(item => {
            ...
            if (oldPriceVal > 0 && priceVal < oldPriceVal) {
                priceHtml = `<span class="old-price">${this.utils.formatPrice(oldPriceVal)}₺</span>${this.utils.formatPrice(priceVal)}₺`;
            } else {
                priceHtml = `${product.isFree ? 'Hediye' : this.utils.formatPrice(priceVal) + '₺'}`;
            }
    ...
    openGalleryInfoModal: function (defaultType = 'komsu', productId = null) {
        console.log('[Gallery Modal] openGalleryInfoModal çağrıldı, defaultType:', defaultType, 'productId:', productId);
        ...
        console.log('[Gallery Modal] Modal element:', modal);
        if (modal) {
            // Modal'ı body'nin en sonuna taşı (parent container sorunlarını bypass)
            if (modal.parentElement !== document.body) {
                document.body.appendChild(modal);
                console.log('[Gallery Modal] Modal document.body\'ye taşındı');
            }

            // Tüm gerekli stilleri açıkça ayarla
            modal.style.cssText = 'display: flex !important; z-index: 11000 !important; position: fixed !important; top: 0 !important; left: 0 !important; right: 0 !important; bottom: 0 !important; width: 100vw !important; height: 100vh !important; background: rgba(0,0,0,0.6) !important; justify-content: center !important; align-items: center !important; visibility: visible !important; opacity: 1 !important;';
            console.log('[Gallery Modal] Modal display: flex olarak ayarlandı, computed style:', window.getComputedStyle(modal).display);
        } else {
            console.error('[Gallery Modal] HATA: modal-gallery-info elementi bulunamadı!');
        }
    ...
    closeGalleryInfoModal: function () {
        const modal = document.getElementById('modal-gallery-info');
        if (modal) {
            modal.style.cssText = 'display: none; z-index: 11000;';
        }
        this.state.currentVitrineProductId = null;
    },

    openPazarAddModal: function () {
        console.log('[Pazar Modal] openPazarAddModal çağrıldı');
        if (!this.checkAuth('Pazar\'a ürün eklemek için lütfen giriş yapın.')) {
            console.log('[Pazar Modal] checkAuth başarısız, kullanıcı giriş yapmamış');
            return;
        }
        console.log('[Pazar Modal] checkAuth başarılı');
        const modal = document.getElementById('pazar-add-modal');
        console.log('[Pazar Modal] Modal element:', modal);
        if (modal) {
            this.state.tempPazarImage = null; // Eski veriyi temizle

            // Modal'ı body'nin en sonuna taşı (parent container sorunlarını bypass)
            if (modal.parentElement !== document.body) {
                document.body.appendChild(modal);
                console.log('[Pazar Modal] Modal document.body\'ye taşındı');
            }

            // Tüm gerekli stilleri açıkça ayarla
            modal.style.cssText = 'display: flex !important; z-index: 99999 !important; position: fixed !important; top: 0 !important; left: 0 !important; right: 0 !important; bottom: 0 !important; width: 100vw !important; height: 100vh !important; background: rgba(0,0,0,0.6) !important; justify-content: center !important; align-items: center !important; visibility: visible !important; opacity: 1 !important;';
            console.log('[Pazar Modal] Modal display: flex olarak ayarlandı, computed style:', window.getComputedStyle(modal).display);
            this.renderPazarCities();
        } else {
            console.error('[Pazar Modal] HATA: pazar-add-modal elementi bulunamadı!');
        }
    },
    ...
    closePazarAddModal: function () {
        ...
        const cityEl = document.getElementById('pazar-city');
        const distEl = document.getElementById('pazar-district');
        const neighEl = document.getElementById('pazar-neighborhood');

        if (titleEl) titleEl.value = '';
        if (priceEl) priceEl.value = '';
        if (descEl) descEl.value = '';
        if (cityEl) cityEl.value = '';
        if (distEl) { distEl.value = ''; distEl.innerHTML = '<option value="">İlçe Seçin</option>'; }
        if (neighEl) { neighEl.value = ''; neighEl.innerHTML = '<option value="">Mahalle Seçin</option>'; }
        if (previewEl) {
            previewEl.src = '';
    ...
    initLiveMap: function () {
        ...
        // Heatmap Toggle Container (Harita zerinde yzen buton)
    ...
    addHeatmapToggle: function () {
        if (!this.liveMapInstance || document.getElementById('heatmap-toggle-btn')) return;

        const toggleContainer = L.control({ position: 'topleft' });
        toggleContainer.onAdd = function () {
            const div = L.DomUtil.create('div', 'heatmap-control-wrapper');
            div.innerHTML = `
                <button id="heatmap-toggle-btn" onclick="app.toggleHeatmap()" class="map-floating-pill">
                    <i class="fas fa-fire"></i> <span>Isı Haritası</span>
                </button>
            `;
            return div;
        };
        toggleContainer.addTo(this.liveMapInstance);
    },
    ...
    toggleHeatmap: function () {
        ...
        if (this.heatmapActive) {
            ...
            this.renderHeatmap();
            this.toast('Aktiflik ısı haritası açıldı.', 'info');
        } else {
    ...
    renderHeatmap: function () {
        ...
        // lan yoğunluğuna gre halkalar oluştur
        this.state.products.forEach(p => {
            const lat = p.lat || p.location?.lat;
            const lng = p.lng || p.location?.lng;
            if (lat && lng && !this.utils.isProductExpired(p)) {
    ...
    clearHeatmap: function () {
        this.heatmapLayers.forEach(layer => this.liveMapInstance.removeLayer(layer));
        this.heatmapLayers = [];
    },

    updateRadarStats: function () {
        const activeCount = this.state.products.filter(p => {
        ...
        const activeEl = document.getElementById('radar-active-count');
        if (activeEl) activeEl.innerText = activeCount;

        // Ayrıca mini radar başlığını da güncelle (V4 dizaynı için)
        const radarStatusEl = document.getElementById('mini-radar-status');
        if (radarStatusEl) radarStatusEl.innerText = `${activeCount} aktif ilan çevrenizde`;
    },

    renderMapMarkers: function () {
        if (!this.liveMapInstance || !this.liveMapMarkersCluster) return;

        // Eski markerları haritadan temizle
        ...
        // Kategori -> İkon eşleşmesi
        const getCategoryIcon = (category) => {
            const icons = {
                'depo': 'fa-warehouse',
                'motosiklet': 'fa-motorcycle',
                'telefon': 'fa-mobile-alt',
                'computer': 'fa-laptop',
                'elektronik': 'fa-plug',
                'evesyasi': 'fa-couch',
                'mobilya': 'fa-chair',
                'beyazesya': 'fa-refrigerator',
                'giyim': 'fa-shirt',
                'ayakkabi': 'fa-shoe-prints',
                'bebek': 'fa-baby-carriage',
                'spor': 'fa-volleyball',
                'oyun': 'fa-gamepad',
                'kitap': 'fa-book',
                'surpriz-paket': 'fa-box',
                'ustayardim': 'fa-tools',
                'Askıda': 'fa-hand-holding-heart'
            };
        ...
        this.state.products.forEach(p => {
            ...
            if (lat && lng) {
                ...
                const customIcon = L.divIcon({
                    html: `
                    <div class="radar-pulse-container pop-in-marker">
                        <div class="radar-dot-category" style="border-color: ${p.isFree ? '#10b981' : 'var(--primary)'};">
                            <i class="fas ${categoryIcon}" style="color: ${p.isFree ? '#10b981' : 'var(--primary)'};"></i>
                        </div>
                    </div>
                `,
                    className: 'custom-ad-marker-modern',
                    iconSize: [32, 32],
                    iconAnchor: [16, 16],
                    popupAnchor: [0, -15]
                });
                ...
                const productImg = p.image || (p.images && p.images.length > 0 ? p.images[0] : 'https://via.placeholder.com/150?text=No+Image');

                marker.bindPopup(`
                    <div style="font-family: 'Outfit', sans-serif; padding: 0; min-width: 200px;">
                        <div style="width: 100%; height: 120px; overflow: hidden; background: #eee;">
                            <img src="${productImg}" style="width: 100%; height: 100%; object-fit: cover;">
                        </div>
                        <div style="padding: 12px;">
                            <h4 style="margin: 0 0 4px; font-size: 0.95rem; font-weight: 800; color: #1e293b; line-height: 1.2;">${p.title}</h4>
                            <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 12px;">
                                <span style="font-size: 0.85rem; font-weight: 800; color: ${p.isFree ? '#10b981' : 'var(--primary)'};">
                                    ${p.isFree ? 'ÜCRETSİZ 🌱' : p.price + ' TL'}
                                </span>
                                <span style="font-size: 0.65rem; font-weight: 700; color: #64748b;">
                                    <i class="far fa-clock"></i> ${this.utils.formatRelativeTime(p.createdAt?.toDate ? p.createdAt.toDate() : new Date(p.createdAt))}
                                </span>
                            </div>
                            <button onclick="app.showProductDetails('${p.id}')" 
                                    class="game-btn-primary" 
                                    style="width: 100%; padding: 8px; font-size: 0.75rem;">
                                İncele
                            </button>
                        </div>
                    </div>
        ...
        this.renderActiveNeighborsOnMap();
        this.renderSafePointsOnMap();
    },

    renderActiveNeighborsOnMap: function () {
        if (!this.liveMapInstance) return;

        const userLoc = (this.state.user && this.state.user.location) ? this.state.user.location : { lat: 41.0082, lng: 28.9784 };

        // Rastgele komşular oluştur (Görsel zenginlik için - ancak ilanlarla karışmaması için farklı stil)
        for (let i = 0; i < 12; i++) {
            const offsetLat = (Math.random() - 0.5) * 0.02;
            const offsetLng = (Math.random() - 0.5) * 0.02;

            const neighborIcon = L.divIcon({
                html: `<div class="radar-dot-neighbor"></div>`,
                className: 'neighbor-marker',
                iconSize: [10, 10],
                iconAnchor: [5, 5]
            });

            const marker = L.marker([userLoc.lat + offsetLat, userLoc.lng + offsetLng], {
                icon: neighborIcon,
                interactive: false // Tıklanamaz, kafa karıştırmaz
            });
            marker.addTo(this.liveMapInstance);
            ...
            if (lat && lng) {
                const customIcon = L.divIcon({
                    ...
                    <div class="radar-pulse-container">
                        <div class="radar-dot-modern" style="background: var(--primary);"></div>
                    </div>
                `,
                    className: 'custom-ad-marker-modern',
                    iconSize: [14, 14],
                ...
                marker.bindPopup(`
                    <div style="font-family: inherit; padding: 5px; min-width: 160px;">
                        ...
                        <button onclick="app.showProductDetails('${p.id}')" 
                                style="width: 100%; background: var(--primary); color: white; border: none; border-radius: 12px; padding: 10px; font-size: 0.8rem; font-weight: 700; cursor: pointer;">
                            İncele
                        </button>
                    </div>
                `);
    ...
    toggleShareViewMode: function () {
        ...
        if (this.state.shareViewMode === 'map') {
            ...
            if (btnEl) btnEl.innerHTML = '<i class="fas fa-list"></i> Liste Modu';
            this.applyShareFilters(); // This will now call renderShareBubbles internally
        } else {
            if (mapEl) mapEl.style.display = 'none';
    ...
    renderShareBubbles: function (posts) {
        ...
        posts.forEach((share, index) => {
            ...
            // Random float duration (20s to 40s) for a slower, calmer feel
            const duration = 20 + Math.random() * 20;
            // Negative delay to stagger starting positions at different heights
    ...
    showShareDetailModal: function (share) {
        // Simple modal to show the post details
        const created = share.createdAt ? new Date(share.createdAt.seconds * 1000) : new Date();
        const timeStr = created.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' });

        let tagClass = `tag-${share.type || 'paylas'}`;
        let tagLabel = share.type === 'paylas' ? 'PAYLAŞIYOR' : (share.type === 'ihtiyac' ? 'ARAŞTIRIYOR' : 'TAKAS EDİYOR');
        if (share.isUrgent) { tagClass = 'tag-urgent'; tagLabel = '🔴 ACİL İHTİYAÇ'; }
        if (share.category === 'Askıda') { tagClass = 'tag-askida'; tagLabel = '🧺 ASKIDA'; }
                ...
                <div class="modal-content game-modal pop-in" onclick="event.stopPropagation()" style="max-width: 400px; padding: 0; overflow: hidden;">
                    <div class="modal-header" style="border-radius: 28px 28px 0 0;">
                         <div style="display: flex; gap: 8px; align-items: center;">
                            <span class="share-tag ${tagClass}" style="font-size: 0.7rem;">${tagLabel}</span>
    ...
    toggleShareCategorySelect: function () {
        ...
        let optionsHtml = categories.map(cat => `
            <div onclick="app.setShareCategoryFilter('${cat}')" class="game-check-card ${cat === current ? 'active' : ''}" style="margin-bottom: 0;">
                <div style="display: flex; align-items: center; gap: 12px;">
                    <i class="fas ${categoryIcons[cat] || 'fa-tag'}" style="width: 20px; text-align: center; color: ${cat === current ? 'white' : 'var(--primary)'};"></i>
                    <span class="game-check-label" style="color: ${cat === current ? 'white' : 'inherit'};">${cat}</span>
                </div>
                ${cat === current ? '<i class="fas fa-check-circle" style="color: white;"></i>' : ''}
            </div>
        `).join('');

        const modalHtml = `
            <div id="category-filter-modal" class="modal-overlay" onclick="this.remove()" style="z-index: 10005; display: flex; align-items: center; justify-content: center;">
                <div class="modal-content game-modal" onclick="event.stopPropagation()">
                    ...
                    <div class="game-modal-body" style="padding: 24px;">
                        <div onclick="app.setShareCategoryFilter(null)" class="game-check-card" style="margin-bottom: 20px; border-color: rgba(225, 29, 72, 0.2); background: rgba(225, 29, 72, 0.02);">
                            <div style="display: flex; align-items: center; gap: 12px; color: #e11d48;">
                                <i class="fas fa-eraser"></i>
                                <span class="game-check-label" style="color: #e11d48;">Filtreyi Temizle</span>
                            </div>
                        </div>
    ...
    renderPazarCities: function () {
        ...
        if (!citySelect) return; // Element yoksa çık (sonsuz döngüyü önle)
    ...
    submitSharePost: async function () {
        ...
        try {
            ...
            // Eğer kullanıcının GPS konumu yoksa (0,0) yerine null veya şehre göre bir tahmin kullan
            if (!shareObj.lat || !shareObj.lng || (shareObj.lat === 0 && shareObj.lng === 0)) {
                // Şimdilik null bırakıyoruz ki filtreleme city/district üzerinden çalışsın
                shareObj.lat = null;
                shareObj.lng = null;
            }
...
`;
    ...
    handleAdTypeChange: function () {
        ...
        if (takasContainer) {
            takasContainer.style.display = 'none';
            document.getElementById('add-takas-wish').value = '';
        }
        ...
        switch (adType) {
            ...
            case 'swap':
                ...
                if (priceInput) priceInput.value = 0; // Backend validasyonu için 0 gönderiyoruz
                break;
            case 'bulk':
    ...
    handleCityChange: function (city) {
        ...
        if (typeof this.updateLiveRadar === 'function') {
            this.updateLiveRadar();
        }
    ...
    filterBySwap: function () {
        this.clearFilters();
        this.state.filters.onlySwap = true;
        this.renderProducts();
        document.getElementById('product-list').scrollIntoView({ behavior: 'smooth' });
        this.toast('Sadece takas ilanları gösteriliyor.');
    },
                ...
                <div style="padding: 16px;">
                        ...
                        <div style="padding: 24px; padding-bottom: 16px; border-bottom: 1px solid #f8fafc;">
                            <div style="display: flex; justify-content: space-between; align-items: flex-start; gap: 12px;">
                                ...
                                <div style="display: flex; align-items: center; gap: 12px;">
                                    ...
                                    <div style="background: ${product.isSwap ? '#6366f1' : (product.isNegotiable ? '#0ea5e9' : '#00ADEF')}; color: white; padding: 10px 18px; border-radius: 16px; font-weight: 800; font-size: 1.1rem; box-shadow: 0 4px 12px ${product.isSwap ? 'rgba(99, 102, 241, 0.25)' : (product.isNegotiable ? 'rgba(14, 165, 233, 0.25)' : 'rgba(0, 173, 239, 0.25)')}; white-space: nowrap;">
                                        ${product.isSwap ? 'TAKAS 🔄' : (product.isNegotiable ? 'TEKLİF 🤝' : (product.isFree ? 'Ücretsiz' : `${this.utils.formatPrice(product.price)}₺`))}
                                    </div>
                                </div>
                            </div>
                    ...
                    : 'Süresi Doldu'}
                            ...
                            <div style="display: flex; flex-direction: column; align-items: center; gap: 4px; flex: 1.5; padding: 0 8px;">
                                ...
                                ${(product.lat || (product.location && product.location.lat)) && (product.lng || (product.location && product.location.lng)) ? `
                                    <button onclick="event.preventDefault(); event.stopPropagation(); window.open('https://www.google.com/maps/dir/?api=1&destination=${product.lat || product.location.lat},${product.lng || product.location.lng}', '_blank')" 
                                        style="margin-top: 4px; background: white; border: 1.5px solid #ef4444; border-radius: 8px; padding: 4px 10px; font-size: 0.65rem; font-weight: 800; color: #ef4444; cursor: pointer; display: flex; align-items: center; gap: 6px; box-shadow: 0 2px 6px rgba(239, 68, 68, 0.1);">
                        ...
                        <div style="padding: 0 24px 24px; display: grid; grid-template-columns: 1.2fr 1fr; gap: 12px;">
                            ${!isOwner && !hasAcceptedOffer && !product.isFree ? `
                                <button onclick="app.openOfferSheet()" 
                                    style="background: ${product.isSwap ? '#6366f1' : '#00ADEF'}; border: none; color: white; padding: 16px; border-radius: 18px; font-weight: 800; font-size: 1rem; box-shadow: 0 6px 15px ${product.isSwap ? 'rgba(99, 102, 241, 0.2)' : 'rgba(0, 173, 239, 0.2)'}; cursor: pointer; transition: all 0.3s; width: 100%;">
                                    <i class="fas ${product.isSwap ? 'fa-right-left' : 'fa-hand-holding-dollar'}"></i> ${product.isSwap ? 'Takas Teklifi Ver' : 'Teklif Ver'}
                                </button>
                            ` : ''}
    ...
    openEditProduct: async function (id) {
        ...
        if (document.getElementById('edit-is-swap')) {
            document.getElementById('edit-is-swap').checked = !!product.isSwap;
            const takasCont = document.getElementById('container-edit-takas-wish');
            if (takasCont) takasCont.style.display = product.isSwap ? 'block' : 'none';
            if (document.getElementById('edit-takas-wish')) {
                document.getElementById('edit-takas-wish').value = product.takasWish || '';
            }
        }
    ...
    toggleRadar: function () {
        ...
        const radarBtn = document.getElementById('radar-btn-side');
    ...
    switchSellerTab: function (tab) {
        ...
        if (tab === 'ads') {
            ...
            if (sellerAds.length === 0) {
                content.innerHTML = `<div class="empty-state-v4">
                    <i class="fas fa-box-open"></i>
                    <p>Aktif ilanı bulunmuyor.</p>
                </div>`;
            } else {
                let adsHtml = '<div class="seller-ads-grid-v4">';
                sellerAds.forEach(p => {
                    const priceText = p.isFree ? 'ÜCRETSİZ 🎁' : `${this.utils.formatPrice(p.price)}₺`;
                    const catIcon = this.utils.getCategoryIcon ? this.utils.getCategoryIcon(p.category) : 'fa-tag';
                    const coverImg = p.image || (p.images && p.images.length > 0 ? p.images[0] : 'https://via.placeholder.com/150?text=No+Image');

                    adsHtml += `
                        <div class="seller-ad-card-v4" onclick="app.closeSellerProfile(); app.showProductDetails('${p.id}')">
                            <div class="seller-ad-card-img-v4" style="background-image: url('${coverImg}')">
                                <div class="seller-ad-card-cat-v4"><i class="fas ${catIcon}"></i></div>
                            </div>
                            <div class="seller-ad-card-info-v4">
                                <div class="seller-ad-card-title-v4">${this.utils.escapeHTML(p.title)}</div>
                                <div class="seller-ad-card-price-v4">${priceText}</div>
                            </div>
                        </div>
                    `;
                });
    ...
    saveSearchAlert: function (type) {
        if (!this.checkAuth('Arama alarmı kurmak için lütfen giriş yapın.')) return;

        const inputId = type === 'share' ? 'share-search-input' : 'main-search-input-field';
        const input = document.getElementById(inputId);
        let query = input ? input.value.trim() : "";

        if (!query && this.state.filters.searchQuery) {
            query = this.state.filters.searchQuery;
        }

        if (!query) {
            this.toast('Lütfen alarm kurmak için bir arama terimi girin.', 'warning');
            return;
        }

        db.collection('user_alerts').add({
            userId: this.state.user.uid,
            query: query.toLowerCase(),
            type: type || 'product',
            createdAt: firebase.firestore.FieldValue.serverTimestamp(),
            userName: this.state.user.name || 'Komşu'
        }).then(() => {
            this.toast(`"${query}" için arama alarmı kuruldu! 🎉`, 'success');
            const alertBtn = document.getElementById('search-alert-btn');
            if (alertBtn) {
            ...
            this.toast('Alarm kaydedilemedi.', 'error');
        });
    },
    ...
    getEsnafLocation: function (event) {
        console.log('[Esnaf Location] getEsnafLocation çağrıldı');
        if (!navigator.geolocation) {
            this.toast('Tarayıcınız konum özelliğini desteklemiyor.', 'error');
            return;
        }

        const button = event ? event.currentTarget : document.querySelector('#modal-esnaf-app button[onclick*="getEsnafLocation"]');
        ...
        navigator.geolocation.getCurrentPosition(
            (pos) => {
                const lat = pos.coords.latitude;
                const lng = pos.coords.longitude;
                console.log('[Esnaf Location] Konum alındı:', lat, lng);

                const latInput = document.getElementById('esnaf-apply-lat');
                const lngInput = document.getElementById('esnaf-apply-lng');
                const locNameInput = document.getElementById('esnaf-apply-loc-name');

                if (latInput) latInput.value = lat;
                if (lngInput) lngInput.value = lng;
                if (locNameInput) locNameInput.value = `${lat.toFixed(4)}, ${lng.toFixed(4)}`;

                if (icon) icon.className = originalClass;
                this.toast('Konum başarıyla alındı! 📍', 'success');
            },
            (err) => {
                console.error('[Esnaf Location] Konum hatası:', err);
                if (icon) icon.className = originalClass;
                ...
                switch (err.code) {
                    ...
                    case err.POSITION_UNAVAILABLE:
                        ...
                        break;
                    case err.TIMEOUT:
                        errorMsg += 'Konum alma işlemi zaman aşımına uğradı.';
                        break;
                    default:
    ...
    // --- Mahalle Borsası / Takas Günü Mantığı ---
    checkTakasDay: function () {
        const urlParams = new URLSearchParams(window.location.search);
        const forceTakas = urlParams.get('takas') === 'true';
        const isSunday = new Date().getDay() === 0;

        if (isSunday || forceTakas) {
            console.log("🤝 Bugün Mahalle Borsası / Takas Günü!");
            this.state.takasDayActive = true;
            document.body.classList.add('takas-day-active');

            // Banner'ı göster
            const banner = document.getElementById('takas-day-banner');
            if (banner) banner.style.display = 'block';

            // İlk girişte bilgi ver (demo için her zaman aktif edelim şimdilik)
            if (!localStorage.getItem('sawTakasIntro')) {
                setTimeout(() => this.showTakasInfo(), 1500);
                localStorage.setItem('sawTakasIntro', 'true');
            }
        }
    },

    showTakasInfo: function () {
        const info = `
            <div style="text-align:center; padding: 10px;">
                <div style="font-size: 3rem; margin-bottom: 15px;">🤝</div>
                <h2 style="color: #4f46e5; margin-bottom: 10px;">Takas Günü'ne Hoş Geldin!</h2>
                <p style="font-size: 0.95rem; color: #475569; line-height: 1.5;">
                    Bugün mahallemizde **para geçmiyor!** <br><br>
                    Eşyalarını komşularının ihtiyacıyla takas edebilir, 
                    hiç para harcamadan yeni şeyler edinebilirsin. 
                    Pazar günü ritüelimize sen de katıl!
                </p>
                <div style="background: #fde68a; padding: 10px; border-radius: 12px; margin-top: 15px; font-size: 0.85rem; font-weight: 700; color: #92400e;">
                    💡 İpucu: İlanların artık fiyat yerine "Takas Tercihi" gösteriyor.
                </div>
            </div>
        `;
        this.openSimpleModal('Mahalle Borsası', info);
    },

    openSimpleModal: function (title, content) {
        const modalHtml = `
            <div id="simple-info-modal" class="modal-overlay" onclick="this.remove()" style="display:flex; align-items:center; justify-content:center; z-index:10005;">
                <div class="modal-content game-modal pop-in" onclick="event.stopPropagation()" style="max-width:380px;">
                    <div class="modal-header">
                        <h3 class="game-modal-title">${title}</h3>
                        <i class="fas fa-times" onclick="this.closest('.modal-overlay').remove()" style="cursor: pointer; font-size: 1.4rem; color: var(--text-muted);"></i>
                    </div>
                    <div class="game-modal-body" style="padding: 24px;">
                        <div style="font-size: 1rem; color: var(--text-main); line-height: 1.6; font-weight: 600;">
                            ${content}
                        </div>
                        <button onclick="this.closest('.modal-overlay').remove()" class="game-btn-primary" style="margin-top: 25px;">Harika, Başlayalım!</button>
                    </div>
                </div>
            </div>
        `;
